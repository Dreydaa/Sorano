...
const onMouseMove = (event) => {
  ...
  if (intersects.length > 0) {
    ...
    if (model !== hoveredModel) {
      ...
      // Nouveau hover : capturer la Y au moment T
      model.userData.baseY = model.position.y
      hoveredModel = model

      if (hoverTween) hoverTween.kill()
      hoverTween = gsap.to(hoveredModel.position, {
        y: model.userData.baseY + 0.3,
        duration: 0.4,
        ease: 'power2.out'
      })
...
window.addEventListener('mousemove', onMouseMove)
  ...
  function triggerModelTransition(cubeIndex) {
    const modelToAnimate = cubesArray.find(cube => cube.userData.index === cubeIndex);
    ...
    const targetSize = viewHeight * 0.90
  ...
  function setupParallaxScroll() {
    ...
    const scrollHeight = Math.max(100, cubesArray.length * 10 * parallaxSettings.scrollDistance)
    ...
    if (!scrollContainer) {
      ...
      scrollContent.style.height = '1000vh'
    ...
    const moveY = tracksData.length * 1
    const moveZ = tracksData.length * 2
  ...
  function animate() {
    ...
    const minZ = -(tracksData.length * 2) - 50;

    cubesArray.forEach(cube => {
      if (cube.position.z > 25 || cube.position.z < minZ) {
        cube.visible = false
      } else {
        cube.visible = true
      }
