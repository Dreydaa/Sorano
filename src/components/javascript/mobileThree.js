...
export default function initMobileScene(canvas, options = {}) {
  ...
  function placeInSlot(model, slotName) {
    ...
    const s = slotName === 'center' ? baseScale * CENTER_SCALE_FACTOR : baseScale * SIDE_SCALE_FACTOR
  ...
  function wrap(index) {
    return ((index % tracksData.length) + tracksData.length) % tracksData.length
  }
  ...
  async function swipeNext() {
    ...
    const tl = gsap.timeline({
      onComplete: async () => {
        if (slots.left.model) scene.remove(slots.left.model)
  ...
  async function swipePrev() {
    ...
    const tl = gsap.timeline({
      onComplete: async () => {
        if (slots.right.model) scene.remove(slots.right.model)
  ...
  async function rebuildCarousel(newCenterIdx, direction) {
    ...
    const modelsPromise = Promise.all([loadModel(leftIdx), loadModel(newCenterIdx), loadModel(rightIdx)])
    const exitKey = direction > 0 ? 'offLeft' : 'offRight'

    const tl = gsap.timeline({
      onComplete: async () => {
        if (slots.left.model) scene.remove(slots.left.model)
        if (slots.center.model) scene.remove(slots.center.model)
        if (slots.right.model) scene.remove(slots.right.model)

        const [newLeft, newCenter, newRight] = await modelsPromise

        if (newLeft) { placeInSlot(newLeft, 'left'); scene.add(newLeft) }
        if (newCenter) { placeInSlot(newCenter, 'center'); scene.add(newCenter) }
        if (newRight) { placeInSlot(newRight, 'right'); scene.add(newRight) }
  ...
  const onTouchEnd = e => {
    ...
    if (Math.abs(dx) < 10 && Math.abs(dy) < 10) {
      ...
      const modelsToIntersect = [slots.left.model, slots.center.model, slots.right.model].filter(Boolean)
      const intersects = raycaster.intersectObjects(modelsToIntersect, true)
      
      if (intersects.length > 0) {
        let clickedModel = intersects[0].object
        while (clickedModel.parent && !modelsToIntersect.includes(clickedModel)) {
          clickedModel = clickedModel.parent
        }
        let clickedIndex = -1
        if (clickedModel === slots.left.model) clickedIndex = slots.left.index
        else if (clickedModel === slots.center.model) clickedIndex = slots.center.index
        else if (clickedModel === slots.right.model) clickedIndex = slots.right.index
        
        if (clickedIndex !== -1 && options.onCubeClick) options.onCubeClick(clickedIndex)
      }
      return
    }
